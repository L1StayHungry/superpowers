...
- **Cursor**：只装 fork。`.cursor-plugin/plugin.json` 加载无报错（含 §5.4 第 7 条 manifest 结构校验通过）；`hooks/hooks-cursor.json` → `hooks/session-start` 链路加载 `skills/t-using-superpowers/SKILL.md`；同上输入等价验证。
- **Codex CLI / App**：只装 fork。
...
- 三个 harness 的插件清单都同时显示 `superpowers` 与 `t-superpowers` 两个独立条目，包管理器无冲突报错。
...
13. 三类并存测试通过：official-only 基线（含 Codex hook 行为记录） /
    fork-only 行为等价（按 harness 拆分）/ co-installed 命名不冲突且可独立启停
```
