我需要修正根级 `AGENTS.md` / `CLAUDE.md`。

当前仓库是基于 `obra/superpowers` fork 的内部二次开发仓库。阶段一命名空间迁移已经完成，但根级 agent 指导文件不应该写成“阶段二施工清单”，也不应该继续默认使用官方 upstream PR contributor guidelines。

请先检查：

1. `AGENTS.md` 是否是指向 `CLAUDE.md` 的 symlink。
2. `docs/upstream-contrib.md` 是否存在。
3. `二次开发规划.md`
4. `阶段一验收记录.md`
5. 当前 `CLAUDE.md`
...
把根级 agent 指导文件改成长期有效的“t-superpowers 二次开发宪章”，而不是阶段二任务列表。

要求：

- 重点声明二次开发初衷、最终期望、长期边界、迭代原则、验证原则、upstream 同步原则。
- 不要把阶段二具体施工清单写进 `AGENTS.md`。
- 可以简短说明“阶段一命名空间迁移已完成”，但不要把阶段二目标逐项展开成任务列表。
- 后续阶段计划应引用 `二次开发规划.md` 和 `阶段一验收记录.md`。
- 默认模式是 internal fork development，不是 upstream PR。
- 只有用户明确要求 “prepare upstream PR / contribute back to upstream / 给官方提 PR” 时，才读取 `docs/upstream-contrib.md` 并进入 upstream PR 模式。
- 如果 `AGENTS.md -> CLAUDE.md` 是 symlink，请保持 symlink 不变，直接重写真实文件 `CLAUDE.md`。
- 如果 `AGENTS.md` 不是 symlink，则重写 `AGENTS.md`，并报告是否需要同步 `CLAUDE.md`。
- 不要修改 `vendor/superpowers/`。
- 不要修改 `docs/upstream-contrib.md`。
- 不要修改 `skills/`。
- 不要修改 `hooks/`。
- 不要修改 plugin manifest。
- 不要开始任何阶段二功能开发。

请将根级 agent 指导文件替换为下面这份内容：

[粘贴下面的 AGENTS.md 内容]

完成后输出：

- `AGENTS.md` 是否仍是 symlink
- 实际修改的文件
- 是否确认 `docs/upstream-contrib.md` 存在
- 本轮未修改的范围
- 下一步建议
```

以下是[ AGENTS.md 推荐内容]，你可以综合判断下是否合适：
