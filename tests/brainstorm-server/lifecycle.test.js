...
const SERVER = path.join(__dirname, '../../skills/t-brainstorming/scripts/server.cjs');
const START = path.join(__dirname, '../../skills/t-brainstorming/scripts/start-server.sh');
const STOP = path.join(__dirname, '../../skills/t-brainstorming/scripts/stop-server.sh');
...
function openCaptureCommandJson(dir, marker) {
  ...
  return JSON.stringify([process.execPath, scriptPath, markerPath]);
}
...
function newestSessionDir(projectDir) {
  const sessionDir = execFileSync('bash', [
    '-lc',
    'find "$1/.t-superpowers/brainstorm" -mindepth 1 -maxdepth 1 -type d -print | sort | tail -1',
    'bash',
    projectDir
  ], { encoding: 'utf8' }).trim();
  assert(sessionDir, `expected at least one session dir under ${projectDir}/.t-superpowers/brainstorm`);
  return sessionDir;
}
...
async function runTests() {
  ...
  await test('auto-opens the browser once, on the first screen', async () => {
    ...
    const openCmd = openCaptureCommandJson(dir, marker); // capture the launch instead of opening a browser
    const srv = spawn('node', [SERVER], { env: { ...process.env, BRAINSTORM_PORT: 3417, BRAINSTORM_DIR: dir, BRAINSTORM_OPEN: '1', BRAINSTORM_OPEN_CMD_JSON: openCmd, BRAINSTORM_LIFECYCLE_CHECK_MS: 100000 } });
    let out = ''; srv.stdout.on('data', d => out += d.toString());
    for (let i = 0; i < 60 && !out.includes('server-started'); i++) await sleep(50);
  ...
  await test('does NOT auto-open unless approved (BRAINSTORM_OPEN unset)', async () => {
    const dir = fs.mkdtempSync('/tmp/bs-open-');
    const marker = path.join(dir, 'opened.log');
    const openCmd = openCaptureCommandJson(dir, marker);
    // BRAINSTORM_OPEN intentionally NOT set — auto-open must stay off.
    const srv = spawn('node', [SERVER], { env: { ...process.env, BRAINSTORM_PORT: 3418, BRAINSTORM_DIR: dir, BRAINSTORM_OPEN_CMD_JSON: openCmd, BRAINSTORM_LIFECYCLE_CHECK_MS: 100000 } });
    let out = ''; srv.stdout.on('data', d => out += d.toString());
    for (let i = 0; i < 60 && !out.includes('server-started'); i++) await sleep(50);
    fs.writeFileSync(path.join(dir, 'content', 'first.html'), '<h2>First</h2>');
    await sleep(700);
