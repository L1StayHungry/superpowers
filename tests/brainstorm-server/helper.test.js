...
const HELPER = path.join(__dirname, '../../skills/t-brainstorming/scripts/helper.js');
...
test('reloads after a brief disconnect so broadcasts missed while offline are recovered', () => {
  const e = makeEnv(); e.boot();
  e.last().open();
  e.last().close();
  e.fireReconnect();
