...
const TEST_PORT = Number(
  process.env.BRAINSTORM_TEST_PORT || (49152 + Math.floor(Math.random() * 16383))
);
...
async function fetchPath(pathname) {
  return new Promise((resolve, reject) => {
    const headers = { Cookie: `brainstorm-key-${TEST_PORT}=${TOKEN}` };
    http.get({ hostname: '127.0.0.1', port: TEST_PORT, path: pathname, headers }, (res) => {
      let data = '';
      res.on('data', chunk => data += chunk);
...
function assertStartedOnExpectedPort(out) {
  ...
  assert.strictEqual(
    msg.port,
    TEST_PORT,
    `server.test.js expected selected port ${TEST_PORT}, got ${msg.port}; tests must not run through fallback`
  );
  return msg;
}
...
async function runTests() {
  ...
  try {
    ...
    await test('rejects raw and encoded traversal instead of normalizing to a content file', async () => {
      fs.writeFileSync(path.join(CONTENT_DIR, 'traversal-target.txt'), 'must not be served through traversal');
      const raw = await fetchPath('/files/../traversal-target.txt');
      const encoded = await fetchPath('/files/%2e%2e/traversal-target.txt');
      assert.strictEqual(raw.status, 404, 'raw traversal must be rejected');
      assert.strictEqual(encoded.status, 404, 'encoded traversal must be rejected');
      assert(!raw.body.includes('must not be served'));
      assert(!encoded.body.includes('must not be served'));
    });
