...
const TEST_PORT = Number(
  process.env.BRAINSTORM_AUTH_TEST_PORT || (49152 + Math.floor(Math.random() * 16383))
);
...
function assertStartedOnExpectedPort(out) {
  ...
  assert.strictEqual(
    msg.port,
    TEST_PORT,
    `auth.test.js expected selected port ${TEST_PORT}, got ${msg.port}; tests must not run through fallback`
  );
  return msg;
}
