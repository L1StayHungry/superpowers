...
import { cursorTarget, doctorCursor, installCursor } from '../../lib/targets/cursor.mjs';

const payloadRoot = path.resolve('dist/npm-package');

function hasBuiltPayload() {
  return (
    existsSync(path.join(payloadRoot, 'package.json')) &&
    existsSync(path.join(payloadRoot, '.cursor-plugin/plugin.json')) &&
    existsSync(path.join(payloadRoot, 'skills/t-brainstorming/SKILL.md')) &&
    existsSync(path.join(payloadRoot, 'hooks/hooks-cursor.json'))
  );
}

before(() => {
  if (hasBuiltPayload()) return;
