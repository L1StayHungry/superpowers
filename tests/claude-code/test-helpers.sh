...
# Resolve the plugin root from this helpers file, even when tests cd elsewhere.
CLAUDE_CODE_TEST_PLUGIN_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
...
run_claude() {
    ...
    # Build command
    local cmd="claude -p \"$prompt\" --plugin-dir \"$CLAUDE_CODE_TEST_PLUGIN_DIR\""
