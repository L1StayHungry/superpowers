...
mkdir -p docsDev/changes/test-feature-design

# Create a spec document WITH INTENTIONAL ERRORS for the reviewer to catch
cat > docsDev/changes/test-feature-design/spec.md <<'EOF'
# Test Feature Design
...
Read the spec-document-reviewer-prompt.md template in skills/t-brainstorming/ to understand the review format.

Then review the spec at $TEST_PROJECT/docsDev/changes/test-feature-design/spec.md using the criteria from that template.
