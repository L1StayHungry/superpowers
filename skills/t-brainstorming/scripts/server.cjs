...
function decodeFrame(buffer) {
  ...
  if ((firstByte & 0x80) === 0) {
    throw new Error('Fragmented WebSocket frames are not supported; FIN must be set');
  }
  if ((firstByte & 0x70) !== 0) {
    throw new Error('WebSocket RSV bits must be zero');
  }
...
<html>
<head><meta charset="utf-8"><title>T-Superpowers Brainstorming</title>
<style>
...
<body><!-- BRANDING --><h1>T-Superpowers Brainstorming</h1>
...
function brandMarkup() {
  ...
  return '<div class="brand"><span class="brand-copy">T-Superpowers Brainstorming v' + version + '</span></div>';
}
...
function getNewestScreen() {
  let names;
  try {
    names = fs.readdirSync(CONTENT_DIR);
  } catch (e) {
    return null;
  }
  const files = names
    ...
    .map(f => {
      ...
      const opened = readContentFileSafely(fp, 'utf8');
      if (!opened) return null;
      return { ...opened, path: fp, mtime: opened.stat.mtime.getTime() };
    })
    .filter(Boolean)
...
function browserLauncherFromCommandJson(raw, url) {
  let command;
  try {
    command = JSON.parse(raw);
  } catch (e) {
    throw new Error('BRAINSTORM_OPEN_CMD_JSON must be a JSON array of argv strings');
  }
  if (!Array.isArray(command)) {
    throw new Error('BRAINSTORM_OPEN_CMD_JSON must be a JSON array of argv strings');
  }
  if (command.length === 0 || typeof command[0] !== 'string' || !command[0]) {
    throw new Error('BRAINSTORM_OPEN_CMD_JSON requires a non-empty executable');
  }
  if (!command.every(arg => typeof arg === 'string')) {
    throw new Error('BRAINSTORM_OPEN_CMD_JSON arguments must all be strings');
  }
  return { bin: command[0], args: [...command.slice(1), url] };
}
...
// Validate and read through one descriptor so pathname swaps cannot redirect a
// request after validation. Hooks exist solely for deterministic race tests.
function readContentFileSafely(filePath, encoding = null, hooks = {}) {
  let fd = null;
  try {
    const realContentDir = fs.realpathSync(CONTENT_DIR);
    const initialPathStat = fs.lstatSync(filePath);
    if (!isSingleLinkRegularFile(initialPathStat)) return null;

    const initialRealPath = fs.realpathSync(filePath);
    if (!isStrictlyInside(realContentDir, initialRealPath)) return null;
    const initialRealStat = fs.statSync(initialRealPath);
    if (!isSingleLinkRegularFile(initialRealStat) ||
        !sameFileIdentity(initialPathStat, initialRealStat)) return null;

    if (typeof hooks.beforeOpen === 'function') hooks.beforeOpen();

    let flags = fs.constants.O_RDONLY;
    if (typeof fs.constants.O_NOFOLLOW === 'number') flags |= fs.constants.O_NOFOLLOW;
    fd = fs.openSync(filePath, flags);

    if (typeof hooks.afterOpen === 'function') hooks.afterOpen(fd);
    ...
    const currentRealStat = fs.statSync(currentRealPath);
    if (!isSingleLinkRegularFile(descriptorStat) ||
        !isSingleLinkRegularFile(currentPathStat) ||
        !isSingleLinkRegularFile(currentRealStat) ||
        !isStrictlyInside(realContentDir, currentRealPath) ||
        !sameFileIdentity(initialPathStat, descriptorStat) ||
        !sameFileIdentity(descriptorStat, currentPathStat) ||
        !sameFileIdentity(descriptorStat, currentRealStat)) return null;

    const data = encoding === null
      ? fs.readFileSync(fd)
      : fs.readFileSync(fd, encoding);
    return { data, stat: descriptorStat };
  } catch (e) {
    return null;
  } finally {
    if (fd !== null) {
...
function handleRequest(req, res) {
  ...
  if (req.method === 'GET' && pathname === '/' && keyFromQuery && timingSafeEqualStr(keyFromQuery, TOKEN)) {
    ...
    let html = screen
      ? (raw => isFullDocument(raw) ? raw : wrapInFrame(raw))(screen.data)
      : waitingPage();
    ...
    let fileName;
    try {
      fileName = decodeURIComponent(pathname.slice(7));
    } catch (e) {
      fileName = '';
    }
    const filePath = path.join(CONTENT_DIR, fileName);
    // Only direct, non-dotfile children are valid. Reject path separators before
    // normalization so raw and encoded traversal cannot alias a safe content file.
    if (!fileName || fileName.startsWith('.') || /[\\/]/.test(fileName) ||
        path.basename(fileName) !== fileName) {
      res.writeHead(404, securityHeaders());
      res.end('Not found');
      return;
    }
    const opened = readContentFileSafely(filePath);
    if (!opened) {
      res.writeHead(404, securityHeaders());
      res.end('Not found');
      return;
...
// browser is already connected. Tests/operators may provide a JSON argv array
// through BRAINSTORM_OPEN_CMD_JSON; every launcher still uses execFile (no shell).
...
function maybeOpenBrowser() {
  ...
  let launcher;
  try {
    launcher = process.env.BRAINSTORM_OPEN_CMD_JSON
      ? browserLauncherFromCommandJson(process.env.BRAINSTORM_OPEN_CMD_JSON, url)
      : browserLauncherForPlatform(url);
  } catch (e) {
    process.stderr.write(JSON.stringify({ type: 'browser-open-error', error: e.message }) + '\n');
    return;
  }
