...
#
# Options:
#   --project-dir <path>  Store session files under <path>/.t-superpowers/brainstorm/
...
if [[ -n "$PROJECT_DIR" ]]; then
  SESSION_DIR="${PROJECT_DIR}/.t-superpowers/brainstorm/${SESSION_ID}"
  # Persist the bound port and key per project so a restart reuses them and an
  # already-open browser tab reconnects to the same URL with a valid cookie.
  export BRAINSTORM_PORT_FILE="${PROJECT_DIR}/.t-superpowers/brainstorm/.last-port"
  export BRAINSTORM_TOKEN_FILE="${PROJECT_DIR}/.t-superpowers/brainstorm/.last-token"
else
  TEMP_ROOT="${TMPDIR:-/tmp}"
  TEMP_ROOT="${TEMP_ROOT%/}"
  SESSION_DIR="${TEMP_ROOT}/t-superpowers-brainstorm/${SESSION_ID}"
fi
...
if [[ -z "$PROJECT_DIR" ]]; then
  printf '%s\n' 't-superpowers-brainstorm-cleanup-v1' > "${STATE_DIR}/cleanup-marker"
  chmod 600 "${STATE_DIR}/cleanup-marker" 2>/dev/null || true
fi
