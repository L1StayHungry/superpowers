...
import { backupDirectory } from '../backups.mjs';
...
export async function updateCursor(options = {}) {
  ...
  if (!targetExists(target) || isSymlink(target)) return installCursor(options);

  const receipt = readReceipt(target);
  if (!isManagedReceipt(receipt, 'cursor')) return installCursor(options);
  ...
  if (receipt.version === nextVersion && !options.force) {
    if (options.dryRun) {
      return {
        ...
        changed: false,
        dryRun: true,
        message: 'Cursor plugin already matches the running package version'
      };
    }
    const result = await doctorCursor({ home, payloadRoot });
    return { ...result, changed: false, message: 'Cursor plugin already matches the running package version' };
  }
  ...
  if (options.dryRun) {
    return {
      ...
      changed: false,
      dryRun: true,
      message: `would back up and update Cursor plugin from ${receipt.version} to ${nextVersion}`
    };
  }

  const backup = backupDirectory({ source: target, home, target: 'cursor', version: receipt.version });
