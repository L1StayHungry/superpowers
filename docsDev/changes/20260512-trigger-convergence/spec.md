...
updated_at: 2026-05-12T01:35:57Z
owner: lihuajun
---
...
- Ambiguous but potentially complex request: ask one concise question or state the boundary, rather than automatically entering the full workflow.
...
- Then the relevant `t-*` skill may be implicitly selected
...
##### Requirement: T-Superpowers Trigger Boundary

The system must require `t-superpowers` skills for explicit requests and complex development work, but must not force `t-superpowers` for simple edits, Q&A, pure code reading, or small mechanical changes.
...
- Given a user asks for explicit `t-superpowers` usage, a multi-file feature, a behavior change, or a root-cause-unknown bugfix
